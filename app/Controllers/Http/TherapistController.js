...
class TherapistController {
  async register({ request, response }) {
    ...
    try {
      ...
      (therapist.workplace = workplace), (therapist.address = address);
  ...
  async getSingleTherapist({ params, response }) {
    ...
    try {
      const get_therapist = await User.query()
        .where("id", user_id)
  ...
  async therapistProfile({ response, auth }) {
    try {
      ...
      const therapist = await User.query()
        .where("id", authUser.id)
        .with("therapist", builder => {
          builder.with("therapist_specializations").with("patients");
        })
        .first();
  ...
  async viewTherapistDashboard({ response, auth }) {
    try {
      const authUser = auth.current.user;
  ...
  async addPatient({ request, response, auth }) {
    ...
    try {
      ...
      const checkParent = await User.query()
  ...
  async viewTherapistPatient({ response, auth }) {
    try {
      ...
      const patients = await Patient.query()
        .where("therapist_id", therapistData.id)
        .andWhere("is_deleted", false)
        .with("therapist")
        .with("parent")
        .fetch();
